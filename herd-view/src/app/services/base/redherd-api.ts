...
export abstract class RedHerdApi {
  ...
  protected getAll<T>(entity: RedHerdEntity): Observable<T[]> {
    ...
    const url = this.context.Token ? `${this.context.Url}?t=${this.context.Token}` : `${this.context.Url}`;
  ...
  protected get<T>(entity: RedHerdEntity, param: number | string): Observable<T> {
    ...
    const url = this.context.Token ? `${this.context.Url}/${param}?t=${this.context.Token}` : `${this.context.Url}/${param}`;
    const options = this.context.HttpOptions;
  ...
  protected delete(id: number): Observable<boolean> {
    ...
    const url = this.context.Token ? `${this.context.Url}/${id}?t=${this.context.Token}` : `${this.context.Url}/${id}`;
    const options = this.context.HttpOptions;
  ...
  protected update(object: RedHerdObject): Observable<boolean> {
    ...
    const url = this.context.Token ? `${this.context.Url}/${object.id}?t=${this.context.Token}` : `${this.context.Url}/${object.id}`;
    const options = this.context.HttpOptions;
