...
var argv = require('optimist')
  .usage([
    'USAGE: $0 [-s <address>] [-p <port>] [-d <directory>]']
  )
  .option('address', {
    alias: 's',
    'default': "0.0.0.0",
...
var startServer = function (app, address, port) {
  app.listen(port, address);
  C.logger.info('listening on ' + address + ':' + port);
};
...
startServer(app, argv.address, +argv.port);
