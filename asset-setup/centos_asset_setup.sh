...
function InstallDependencies {
        ...
        yum makecache
        yum install curl psmisc openssh-server openvpn jq lftp -y
}
...
function InstallCA {
        curl -k -u $USERNAME:$PASSWORD https://$DSTRSRV_PUBLIC_ADDRESS:$PUBLIC_DSTRSRV_PORT/$ASSET_FINGERPRINT/ca.crt > /etc/pki/ca-trust/source/anchors/$CERT_NAME.crt
        /bin/update-ca-trust extract
}

function RemoveCA {
        rm -rf /etc/pki/ca-trust/source/anchors/$CERT_NAME.crt
        /bin/update-ca-trust 
}
...
function AddCronJob {
        ...
        systemctl reload crond.service > /dev/null 2> /dev/null
}
...
function AddUser {
        ...
        /usr/sbin/usermod -aG wheel $SUDO_USER_USERNAME
...
function ConfigureFirewall {
        ...
        echo -e "$GREEN $BOLD [!] Adding tun0 to ${DEFAULT_ZONE} $RESET"
        firewall-cmd --zone=${DEFAULT_ZONE} --permanent --add-interface=tun0
        echo -e "$GREEN $BOLD [!] Adding IPv4 rule $RESET"
        firewall-cmd --zone=${DEFAULT_ZONE} --permanent --add-rich-rule 'rule family="ipv4" source address="0.0.0.0/0" accept'
        echo -e "$GREEN $BOLD [!] Adding IPv6 rule $RESET"
        firewall-cmd --zone=${DEFAULT_ZONE} --permanent --add-rich-rule 'rule family="ipv6" source address="::/0" accept' --permanent
        echo -e "$GREEN $BOLD [!] Reloading $RESET"
        firewall-cmd --reload
        
        # Inspect all rules for the default zone
        #firewall-cmd --list-all --zone=${DEFAULT_ZONE} --permanent
}

function ResetFirewall {
        DEFAULT_ZONE=$(firewall-cmd --get-default-zone)

        echo -e "$YELLOW $BOLD [!] Removing tun0 from ${DEFAULT_ZONE} $RESET"
        firewall-cmd --zone=${DEFAULT_ZONE} --permanent --remove-interface=tun0
        echo -e "$YELLOW $BOLD [!] Removing IPv4 rule $RESET"   
        firewall-cmd --zone=${DEFAULT_ZONE} --permanent --remove-rich-rule 'rule family="ipv4" source address="0.0.0.0/0" accept'
        echo -e "$YELLOW $BOLD [!] Removing IPv4 rule $RESET"
        firewall-cmd --zone=${DEFAULT_ZONE} --permanent --remove-rich-rule 'rule family="ipv6" source address="::/0" accept'
        echo -e "$YELLOW $BOLD [!] Reloading $RESET"
        firewall-cmd --reload
}
